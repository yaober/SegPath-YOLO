...
def export_detections_to_image(res, img_size, labels_color, save_masks=True, border=3, alpha=1.0):
    ...
    if not len(res['boxes']):  # empty results
        return np.zeros((h_s, w_s, 4), dtype=np.uint8)

    max_val = int(res['labels'].max())
    ...
    for idx, (box, label) in enumerate(zip(res['boxes'], res['labels'])):
        b_0, b_1, b_2, b_3 = box.round().to(torch.int32)
        w = max((b_2 - b_0 + 1).item(), 1)
        h = max((b_3 - b_1 + 1).item(), 1)
        x_0 = max(b_0, 0)
        x_1 = min(b_2 + 1, w_s)
        y_0 = max(b_1, 0)
        y_1 = min(b_3 + 1, h_s)
        ...
        if label > 0 and x_0 < x_1 and y_0 < y_1:  # ignore unclassified and out of bounds
            ...
            if save_masks and 'masks' in res:  # draw mask
                mask = res['masks'][idx]
                mask_obj = np.zeros((h, w), dtype=np.uint8)
                if len(mask):  # Segmentation fault (core dumped)
                    pts = (mask - [b_0, b_1]).astype(np.int32)
                    cv2.fillPoly(mask_obj, pts=pts[None], color=col_label)
